...
package Yaml.Text is
   ...
   type Pool is new Ada.Finalization.Controlled with private;
...
private
   ...
   function Hold (Content : String) return Constant_Instance is
     ((H => (Pool => null, First => 1, Last => Content'Length, others => <>),
       S => Content & (Content'Length + 1 .. 128 => <>)));

   Empty_Holder : constant Constant_Instance := Hold ("");

   type Pool is new Ada.Finalization.Controlled with record
      Data : Pool_Data_Access;
   end record with Type_Invariant => Pool.Data.Pos mod Header_Size = 1;

   overriding procedure Adjust (Object : in out Pool);
   ...
   --  for debugging
   function Current_Chunk_As_String (P : Pool) return String;
